...
import { MyTicketInboxContext } from "./myTicketInboxContext";
import {
  getAllDepartments,
...
export const MyTicketInboxProvider = ({ children }) => {
  ...
  // // Get Allocation Data By Id
  // const getAllocationDataById = async (id) => {
  //   try {
  //     const response = await getAllTicketsAllocatedToUser(id);
  //     if (response.success) {
  //       setData(response.data[0]);
  //     } else {
  //       toast.error(response.message);
  ...
  return (
    <MyTicketInboxContext.Provider value={contextValue}>
      {children}
    </MyTicketInboxContext.Provider>
  );
};
