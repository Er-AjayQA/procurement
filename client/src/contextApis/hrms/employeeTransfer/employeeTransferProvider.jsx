...
export const EmployeeTransferProvider = ({ children }) => {
  ...
  const getAllRequestsGeneratedByMeData = async (selectedEntity) => {
    try {
      setIsLoading(true);
      const finalFilter = getFinalFilter();
      const data = await getAllTransfer(selectedEntity, userDetails?.id, {
        limit,
        page,
        filter: finalFilter,
      });
  ...
  const getAllRequestsApprovalPendingByMeData = async (selectedEntity) => {
    try {
      setIsLoading(true);
      const finalFilter = getFinalFilter();
      const data = await getAllTransfersApprovalPendingByUser(
        selectedEntity,
        userDetails?.id,
        {
          limit,
  ...
  const getAllRequestsApprovedByMeData = async (selectedEntity) => {
    try {
      setIsLoading(true);
      const finalFilter = getFinalFilter();
      const data = await getAllTransfersApprovedByUser(
        selectedEntity,
        userDetails?.id,
        {
          limit,
  ...
  const getAllRequestsRejectedByMeData = async (selectedEntity) => {
    try {
      setIsLoading(true);
      const finalFilter = getFinalFilter();
      const data = await getAllTransfersRejectedByUser(
        selectedEntity,
        userDetails?.id,
        {
          limit,
  ...
  const getAllTransferTypeOptions = async (selectedEntity) => {
    try {
      const response = await getAllTransferType(selectedEntity, {
        limit: 5000,
        page: "",
        filter: {
          transfer_type: "",
  ...
  const getAllTransferReasonOptions = async (selectedEntity) => {
    try {
      const response = await getAllTransferReason(selectedEntity, {
        limit: 5000,
        page: "",
        filter: {
          reason_type: "",
  ...
  // Get All Roles List
  const getAllRolesOptions = async (selectedEntity) => {
    try {
  ...
  const getAllDesignationOptions = async (selectedEntity) => {
    try {
      const response = await getAllDesignations(selectedEntity, {
        limit: 500,
        page: "",
        filter: "",
      });
  ...
  // Get All Branch List
  const getAllBranchOptions = async (selectedEntity) => {
    try {
      const response = await getAllBranches(selectedEntity, {
        limit: 5000,
        page: "",
        filter: {
          billing_status: "",
  ...
  useEffect(() => {
    if (activeEntity && currentTab === "my_requests") {
      getAllRequestsGeneratedByMeData(activeEntity, userDetails?.id);
    } else if (activeEntity && currentTab === "pending_for_approval") {
      getAllRequestsApprovalPendingByMeData(activeEntity, userDetails?.id);
    } else if (activeEntity && currentTab === "approved_by_me") {
      getAllRequestsApprovedByMeData(activeEntity, userDetails?.id);
    } else if (activeEntity && currentTab === "rejected_by_me") {
      getAllRequestsRejectedByMeData(activeEntity, userDetails?.id);
    }
  }, [
    limit,
    page,
  ...
  useEffect(() => {
    if (activeEntity) {
      getAllRolesOptions(activeEntity);
      getAllDepartmentOptions(activeEntity);
      getAllDesignationOptions(activeEntity);
      getAllBranchOptions(activeEntity);
      getAllUsersOptions();
      getAllTransferTypeOptions(activeEntity);
      getAllTransferReasonOptions(activeEntity);
    }
