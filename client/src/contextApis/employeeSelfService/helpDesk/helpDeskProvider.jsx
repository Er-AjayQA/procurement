...
export const HelpDeskProvider = ({ children }) => {
  ...
  const getAllTicketsGeneratedByMeData = async (selectedEntity, id) => {
    try {
      setIsLoading(true);
      const finalFilter = getFinalFilter();
      const data = await getAllTicketsGeneratedByUser(selectedEntity, id, {
        limit,
        page,
        filter: finalFilter,
      });
  ...
  const getAllTicketsAllocatedToMeData = async (selectedEntity, id) => {
    try {
      setIsLoading(true);
      const finalFilter = getFinalFilter();
  ...
  useEffect(() => {
    if (activeEntity && currentTab === "my_requests") {
      getAllTicketsGeneratedByMeData(activeEntity, userDetails?.id);
    } else if (currentTab === "allocate_to_me") {
      getAllTicketsAllocatedToMeData(activeEntity, userDetails?.id);
    }
  }, [
    limit,
    page,
